...
export const ehTickerValido = (ticker) => {
  ...
  if (NAO_E_B3.some(p => t.includes(p))) return false
...
export const parseData = (valor) => {
  ...
  if (matchBr4) {
    return `${matchBr4[3]}-${matchBr4[2].padStart(2,'0')}-${matchBr4[1].padStart(2,'0')}`
  }
...
export const encontrarCampo = (row, nomesPossiveis) => {
  for (const nome of nomesPossiveis) {
    for (const key of Object.keys(row)) {
      const keyNorm = key.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim().replace(/\s+/g, ' ')
      const nomeNorm = nome.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim().replace(/\s+/g, ' ')
...
export const processarOperacoes = (jsonData) => {
  const resultado = []
  const stats = { total: jsonData.length, aceitas: 0, sem_ticker_e_data: 0, nao_e_b3: 0, sem_valor: 0, sem_qtde: 0, data_invalida: 0 }
  console.log('[OPERAÇÕES] Linhas:', jsonData.length)
  for (const row of jsonData) {
    ...
    if (!ticker && !data) { stats.sem_ticker_e_data++; continue }
    if (!ticker || !data) { stats.sem_ticker_e_data++; continue }
    if (!ehTickerValido(ticker)) { stats.nao_e_b3++; continue }
    ...
    if (!tickerLimpo) { stats.nao_e_b3++; continue }
    if (!dataLimpa || !ehDataValida(dataLimpa)) { stats.data_invalida++; continue }
    if (qtdeNum === 0) { stats.sem_qtde++; continue }
    if (valorNum === 0) { stats.sem_valor++; continue }
    let tipoOp = 'COMPRA'
    if (opStr.includes('VENDA') || opStr === 'V' || qtdeNum < 0) tipoOp = 'VENDA'
    else if (opStr.includes('COMPRA') || opStr === 'C') tipoOp = 'COMPRA'
    resultado.push({
      data: dataLimpa, ticker: tickerLimpo, quantidade: Math.abs(qtdeNum),
      preco_unitario: valorNum, operacao: tipoOp,
      tipo_ativo: identificarTipo(tickerLimpo), selecionado: true,
    })
    stats.aceitas++
  }
  console.log('[OPERAÇÕES] Stats:', stats)
  return resultado
}

export const processarAportes = (jsonData) => {
  const resultado = []
  const stats = { total: jsonData.length, aceitos: 0, sem_data: 0, sem_valor: 0, data_invalida: 0 }
  for (const row of jsonData) {
    const data = encontrarCampo(row, ['Data'])
    const valor = encontrarCampo(row, ['Valor'])
    const descricao = encontrarCampo(row, ['Descricao', 'Descrição', 'Obs'])
    if (!data) { stats.sem_data++; continue }
    if (valor === null || valor === undefined || valor === '') { stats.sem_valor++; continue }
    const dataLimpa = parseData(data)
    const valorNum = parseNumero(valor)
    if (!dataLimpa || !ehDataValida(dataLimpa)) { stats.data_invalida++; continue }
    if (valorNum === 0) { stats.sem_valor++; continue }
    resultado.push({
      data: dataLimpa, valor: valorNum,
      descricao: descricao ? String(descricao) : null, selecionado: true,
    })
    stats.aceitos++
  }
  console.log('[APORTES] Stats:', stats)
  return resultado
}

export const processarDividendos = (jsonData) => {
  const resultado = []
  const stats = { total: jsonData.length, aceitos: 0, sem_ticker: 0, sem_data: 0, sem_valor: 0, nao_e_b3: 0, data_invalida: 0 }
  for (const row of jsonData) {
    ...
    if (!ticker) { stats.sem_ticker++; continue }
    if (!data) { stats.sem_data++; continue }
    if (!ehTickerValido(ticker)) { stats.nao_e_b3++; continue }
    const tickerLimpo = limparTicker(ticker)
    const dataLimpa = parseData(data)
    ...
    if (tipoStr.includes('JUROS') || tipoStr.includes('JCP')) tipoNorm = 'JUROS'
    else if (tipoStr.includes('DIVIDENDO')) tipoNorm = 'DIVIDENDO'
    resultado.push({
      ticker: tickerLimpo, data_pagamento: dataLimpa,
      valor: valorNum, tipo: tipoNorm, selecionado: true,
    })
    stats.aceitos++
  }
  console.log('[DIVIDENDOS] Stats:', stats)
  return resultado
}
